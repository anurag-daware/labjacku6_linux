...
#define GAIN_LJBP10V 0x00 // 0000 0000
#define GAIN_LJBP1V 0x20 // 0001 0000
#define GAIN_LJBP1mV 0x40    // 0010 0000
#define GAIN_LJBP10mV 0x60   // 0011 0000

#define GAIN_LJDIFF 0x80  // 0000 0000
#define GAIN_LJSE 0x00  // 1000 0000


///const uint8 NumChannels = 4;        //For this example to work proper, SamplesPerPacket needs
...
const uint8 NumChannels = 9;        //For this example to work proper, SamplesPerPacket needs
...
const uint8 SamplesPerPacket = 9;

int iGainIndexes[] = { 3, 0, 0, 0, 1, 3, 3, 3, 3 };
...
int StreamConfig_example(HANDLE hDevice)
{
    ...
    sendBuff[7] = 8;                 //ResolutionIndex
    sendBuff[8] = SamplesPerPacket;  //SamplesPerPacket
    sendBuff[9] = 0;                 //Reserved
    sendBuff[10] = 0;                //SettlingFactor: 0

    sendBuff[11] = (0 << 3) + (1 << 1);  //ScanConfig: bit 3 set to 0 (4 MHz), bit 1 set to 1 (divide by 256)

    scanInterval = 15625 / 10;  //clock / scanrate
...
/*   sendBuff[11] = 0;  //ScanConfig:
    ...
    for (i = 0; i < NumChannels; i++)
    {
        ...
        {
        case 0:
            sendBuff[14 + i * 2] = channelIndex;  //ChannelNumber (Positive) = i
            sendBuff[15 + i * 2] = (1 << 7) + (1 << 4) + (1 << 5);  //ChannelOptions:
            printf("ChannelIndex : %d in Differntial Mode.\n : %x\n", channelIndex, sendBuff[15 + i * 2]);
            channelIndex += 2;
            break;
        case 2:
            sendBuff[14 + i * 2] = channelIndex;  //ChannelNumber (Positive) = i
            sendBuff[15 + i * 2] = 0 + (0 << 4);  //ChannelOptions:
            printf("ChannelIndex : %d in Single Ended Mode.\n", channelIndex);
            channelIndex++;
            break;
        case 3:
            sendBuff[14 + i * 2] = channelIndex;  //ChannelNumber (Positive) = i
        ...
        case 5:
            sendBuff[14 + i * 2] = channelIndex;  //ChannelNumber (Positive) = i
            sendBuff[15 + i * 2] = 0 + (0 << 5) + (1 << 4);  //ChannelOptions:
            printf("ChannelIndex : %d in Single Ended Mode.\n", channelIndex);
            channelIndex++;
            break;
        case 6:
        ...
        for (j = 0; j < numReadsPerDisplay; j++)
        {
            ...
            for (m = 0; m < readSizeMultiplier; m++)
            {
                ...
                for (k = 12; k < (12 + SamplesPerPacket * 2); k += 2)
                {
                    ...
                    getAinVoltCalibrated(caliInfo, 8, iGainIndexes[currChannel], 0, voltageBytes, &(voltages[scanNumber][currChannel]));
